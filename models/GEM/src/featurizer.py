...
from src.utils import mol_to_geognn_graph_data_MMFF3d
...
class DownstreamTransformFn(object):
    ...
    def __call__(self, raw_data):
        ...
        code_thread.start()
        code_thread.join(timeout_duration)
        if code_thread.is_alive():
            code_thread.join()  # Wait for the thread to finish
            print("The line of code took too long to execute. Skipping...")
            return None
