...
for cell_line in $cell_line_list;do
    for test_fold in {1..7};do
        for seed in {1..5};do    
            python finetune_regr.py --test_fold $test_fold --seed $seed --cell_line $cell_line --search no_tune --split_type scaffold &> gem_${cell_line}_fold_${test_fold}_seed_${seed}.txt 2>&1 &
        done
    done
    wait
done